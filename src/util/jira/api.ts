...
export async function getUserPageData(emailParam: string, storyPointsFieldId?: string) {
  ...
  const candidates = await jiraFetchJson<JiraUserSearchHit[]>(searchUrl.pathname + searchUrl.search)

  const lower = email.toLowerCase()
  let user = candidates.find(u => u.emailAddress?.toLowerCase() === lower) ?? undefined
